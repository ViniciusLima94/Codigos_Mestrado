...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "cvodeieq");
 	...
 	ivoc_help("help ?1 it /home/vinicius/Códigos Mestrado/Ajuste Modelo GL/FI modelos HH/x86_64/IT_huguenard.mod\n");
...
static int castate (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset=0; int error = 0;
 {
   evaluate_fct ( _threadargscomma_ v ) ;
   Dh = ( h_inf - h ) / tau_h ;
   }
 ...
 v=_v;
{
  cai = _ion_cai;
  cao = _ion_cao;
 {   euler_thread(1, _slist1, _dlist1, _p, castate, _ppvar, _thread, _nt);
     if (secondorder) {
    int _i;
    for (_i = 0; _i < 1; ++_i) {
